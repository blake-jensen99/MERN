...
// console.log(names)
const idBig99 = pokémon.filter(p => p.id > 99).map(p => p.name)
// console.log(idBig99)
const poisonNames = pokémon.filter(p => p.types == 'poison').map(p => p.name)
// console.log(poisonNames)
const firstTypeOfFlying = pokémon.filter(p => p.types[1] == 'flying').map(p => p.types[0])
// console.log(firstTypeOfFlying)
const numberOfNorm = pokémon.filter(p => p.types.includes('normal'))
let count = 0
for (num in numberOfNorm) {
    count += 1
}
console.log(numberOfNorm)
console.log(count)
